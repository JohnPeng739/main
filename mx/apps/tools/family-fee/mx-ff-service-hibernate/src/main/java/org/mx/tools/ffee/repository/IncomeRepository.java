...
public interface IncomeRepository extends Repository<IncomeEntity, String> {
    ...
    @Query("select income from IncomeEntity  income where income.createdTime >= ?1 and income.createdTime < ?2")
    List<Income> listIncomesBetween(long startDatetime, long endDatetime);
}
