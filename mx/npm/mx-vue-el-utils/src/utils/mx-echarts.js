 ...
 */
const createGaugeOption = function (gauge, title, min, max, value, valueTitle, unitTitle) {
  min = min || 0
  max = max || 100
  value = round(value || (Math.random() * 100), 0)
  valueTitle = valueTitle || i18n.t('message.title.value')
  unitTitle = unitTitle || ''
  return {
...
const createPieOption = function (pie, title, items) {
  let dataLegend = []
  let series = []
      ...
      if (sumTotal) {
        option.series[index].markPoint = {
          data: [{
            symbol: 'circle',
            symbolSize: 65,
            name: i18n.t('message.title.total'),
...
const createGraphOption = function (title, nodes, links, type) {
  let nodesData = []
  let linksData = []
