...
public interface GeneralEntityAccessor extends GeneralAccessor {
     ...
     * @return 指定实体的数量
     */
    <T extends Base> long count2(Class<T> entityClass, boolean isInterfaceClass);
     ...
     * @return 指定实体对象类别
     */
    <T extends Base> List<T> list2(Class<T> entityClass, boolean isInterfaceClass);
     ...
     * 进行全文检索
     *
     * @param content          全文匹配内容
     * @param valid            如果设置为true，表示仅搜索有效记录；否则搜索全部记录
     * @param entityClass      实体类
     * @param isInterfaceClass 如果设置为true，表示指定的是实体接口类；否则为实体类
     * @param <T>              操作的范型类型
     * @return 满足条件的实体对象列表
     */
    <T extends Base> List<T> search(String content, boolean valid, Class<T> entityClass, boolean isInterfaceClass);

    /**
     * 进行全文检索
     *
     * @param contents         全文匹配内容列表
     * @param valid            如果设置为true，表示仅搜索有效记录；否则搜索全部记录
     * @param entityClass      实体类
     * @param isInterfaceClass 如果设置为true，表示指定的是实体接口类；否则为实体类
     * @param <T>              操作的范型类型
     * @return 满足条件的实体对象列表
     */
    <T extends Base> List<T> search(List<String> contents, boolean valid, Class<T> entityClass, boolean isInterfaceClass);
     ...
     * @return 满足条件的实体对象列表
     */
    <T extends Base> List<T> search(Pagination pagination, String content, boolean valid, Class<T> entityClass,
     ...
     * @return 满足条件的实体对象列表
     */
    <T extends Base> List<T> search(Pagination pagination, List<String> contents, boolean valid, Class<T> entityClass,
                                    boolean isInterfaceClass);
}
