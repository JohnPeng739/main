...
public class JmsPullSpout extends BaseRichSpout {
	...
	private final int maxQueueSize = 3000;
	...
	public void open(Map conf, TopologyContext context, SpoutOutputCollector collector) {
		...
		Long topologyTimeout = topologyTimeoutConf != null ? topologyTimeoutConf.longValue()
				: DEFAULT_MESSAGE_TIMEOUT_SECS;
		if ((TimeUnit.SECONDS.toMillis(topologyTimeout)) > this.recoveryPeriodMs) {
			if (logger.isWarnEnabled()) {
				logger.warn(String.format(
						"*** WARNING ***: Recovery period (%d ms.) is less then the configured "
	...
	public void close() {
		try {
			...
			if (this.pullMessageTimer != null) {
				this.pullMessageTimer.cancel();
				this.pullMessageTimer.purge();
			}
			if (this.recoveryTimer != null) {
				this.recoveryTimer.cancel();
				this.recoveryTimer.purge();
			}
			this.consumer.close();
			this.session.close();
	...
	public void nextTuple() {
		...
		if (message == null) {
			...
			try {
				...
				if (logger.isDebugEnabled()) {
					logger.debug(String.format("Requested deliveryMode: %s.",
							toDeliveryModeString(message.getJMSDeliveryMode())));
